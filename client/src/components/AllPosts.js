...
export default function AllPosts({ posts, setPosts }) {
  const updateLikes = async (postId) => {
    try {
      const response = await fetch(`${apiKey}/post/likePost/${postId}`, {
        method: "PUT",
  ...
  const deleteComment = async (commentId, postId) => {
    try {
      const response = await fetch(`${apiKey}/post/deleteComment/${commentId}`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        credentials: "include"
      });
  ...
  const deletePost = async (postId) => {
    try {
      const response = await fetch(`${apiKey}/post/deletePost/${postId}`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" },
        credentials: "include"
      })
